...
func CheckStatus(client *http.Client, browser string) (string, bool, error) {
	...
	if err := json.Unmarshal(body, &authStatus); err != nil {
		// If the response is not JSON, then the user is not logged in
		return "", false, err
	}

	return authStatus.Authentication.CustomerID, authStatus.Authentication.Authenticated, nil
}
